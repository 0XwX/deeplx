...
export async function onRequestGet() {
  return new Response('POST to /api/deepl with {"text": "...", "target_lang": "ZH"}', {
...
export async function onRequestPost({ request, env }) {
  ...
  const validation = provider.validateParams({ text, targetLang })
  if (!validation.valid) {
    return json({ code: HTTP_STATUS_BAD_REQUEST, data: validation.error }, HTTP_STATUS_BAD_REQUEST)
  }
  ...
  try {
    ...
    if (result.code === HTTP_STATUS_OK) {
      ...
      console.error(`DeepL translation failed. Code: ${result.code}, Error: ${result.error}`)
      return json({ code: result.code, data: result.error || 'Translation failed' }, result.code)
    }
  } catch (err) {
    console.error('Internal server error in /api/deepl:', err)
    return json(
